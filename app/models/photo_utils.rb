class PhotoUtils
  ...
  def self.temporary_upload_dir(user_id)
    return APP_CONFIG["photo_storage"]["tmp_upload_path"].sub("@@USER_ID@@", user_id.to_s)
  end

  def self.originals_path(user_id)
    return APP_CONFIG["photo_storage"]["originals_path"].sub("@@USER_ID@@", user_id.to_s)
  end

  def self.generated_images_path(user_id)
    return APP_CONFIG["photo_storage"]["generated_images_path"].sub("@@USER_ID@@", user_id.to_s)
  end
  ...
  def self.file_name_for_photo(photo, variant: nil, retry_cnt: nil)
    photo_date = photo.time.to_datetime.utc.to_datetime # it's some activerecord bastard class, get a datetime from it
